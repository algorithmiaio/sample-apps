# Tensorflow MNIST Running On GPU's Demo
...
Then note that there is another file in the repo called [load-mnist-data.py](https://github.com/algorithmiaio/sample-apps/blob/master/algo-dev-demo/tensorflow-mnist-gpu/load-mnist-data.py).

3. Create a file in the directory structure to the left of your code in the Web IDE by clicking on "**+** New File" and call it "load-mnist-data.py" then copy and paste from [load-mnist-data.py](https://github.com/algorithmiaio/sample-apps/blob/master/algo-dev-demo/tensorflow-mnist-gpu/load-mnist-data.py).
